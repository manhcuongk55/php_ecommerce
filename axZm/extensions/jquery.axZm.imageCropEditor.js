...
;(function($) {
	...
	sortUpdate = function(e, ui){
		...
		$.each(jcrop_az.order, function(k, v){
			...
			if (t.length == 1){t.detach().appendTo(tP);}
		});
	...
	getAllCropsJSON = function(q, cache, dim, prc){
		...
		$.each(jcrop_az.order, function(a, b){
			...
			var tTbl = $('table[data-cropNum='+k+']', dW),
				tTitle = $('input[type=text]', tTbl).val(),
				tDescr = $('textarea', tTbl).val();
			
			if (tTitle){
				obj.title = tTitle.replace(/\"/g,'&#34;');
			}
			
			if (tDescr){
				obj.descr = tDescr.replace(/\"/g,'&#34;');
			}
	...
	deleteButton = function(num){
		...
		.bind('click', function(e){
			...
			$('#cropSlider').axZmThumbSlider('removeThumb', '[data-cropNum='+toDelNum+']');
			
			// Delete description
			$('#aZcR_descrWrap>table[data-cropNum='+toDelNum+']').remove();
			
			// Delete data stored in global object
			delete jcrop_az.allCrops[toDelNum];
			
			// remove this thumb (not physically)
			$('#realCropThumb_'+toDelNum).parent().axZmRemove();
	...
	toggleWYSIWYG = function(num){
		...
		if (!o.data('cleditor')){
			...
			o.parent().remove();
			$('<textarea style="width: 100%; height: 150px">'+v+'</textarea>').appendTo(p);
		}
	},
	
	addCropDescrTable = function(img, num){
		// Add thumb description to #aZcR_descrWrap
		var title = jcrop_az.allCrops[num].title || '',
		...
		var table = '<table cellspacing="5" cellpadding="0" width="100%" style="border: 1px solid #aaaaaa; margin-bottom: 10px" data-cropNum="'+num+'">\
		<tbody><tr>\
		<td style="vertical-align: top; width: 110px;">\
		<img src="'+img+'" style="max-width: 100px; height: auto; margin-top: 15px; border: 2px groove #ffffff; cursor: pointer;" onclick="$(\'#cropSlider\').axZmThumbSlider(\'triggerClick\', '+num+'); $(window).scrollTo(0)">\
		<br><input type="button" value="WYSIWYG" style="width: 88px; margin-left: 7px;" onclick="$.aZcropEd.toggleWYSIWYG('+num+')">\
		</td>\
		<td style="vertical-align: top;">\
		<label>Title:</label>\
		<input type="text" style="width: 100%; margin-bottom: 5px;" value="'+title+'">\
		<label style="white-space: nowrap">Description (text/html/css and javascript):</label>\
		<textarea style="width: 100%; height: 150px">'+descr+'</textarea>\
		</td></tr></tbody></table>';
	...
	proceedJSONdata = function(d){
		...
		$(d).each( function(m, o) {
			...
			$(tempImg).load(imgCropperPath, null, function(responseText, textStatus, jqXHR){
				// Read header content type
				var contentType = jqXHR.getResponseHeader('Content-Type');
	...
	saveJSONtoFile = function(){
		$("#aZcR_saveJSON").unbind('submit').bind('submit', function(event){
			...
			var Form = $(this),
				...
				keepFormat = $('#aZcR_jsKeepFormated').axZmGetPropType('checked') ? 1 : 0,
				fileName = $('#aZcR_jsFileName').val().replace(/[^a-zA-Z0-9-_]/g, ''),
				password = $('#aZcR_jsFilePass').val(),
				backup = $('#aZcR_jsBackUp').axZmGetPropType('checked') ? 1 : 0,
	...
	jQuery(document).ready(function() {
		...
		jQuery('#cropSlider').axZmThumbSlider({
			...
			/*btnFwdStyle: {lineHeight: '56px', borderRadius: 0},
			btnBwdStyle: {lineHeight: '56px', borderRadius: 0},
			thumbLiStyle: {
				height: 46,
				width: 46,
				lineHeight: 46,
