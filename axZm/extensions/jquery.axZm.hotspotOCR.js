...
;(function($){
	...
	var pageSearchToggle = function(inpID){
 		...
 		$('#hotspotCoord').empty();
 		$('.axZm_zoomGalleryBox, .axZm_zoomFullGalleryBox').css('display', 'block');
	...
	var handleAutoSuggest = function(){
		...
		if (countMatches <= 0){
			$('.axZm_zoomGalleryBox').css('display', 'block').css('marginTop', 0);
			$('.axZm_zoomFullGalleryBox').css('display', 'block');
			$('#axZm_zoomGal_1').css('marginTop', 6);
			$('#someTextLeft').css('display', 'block');
		}else{
			$('.axZm_zoomGalleryBox').css('display', 'none');
			$('.axZm_zoomFullGalleryBox').css('display', 'none');
		...
		$.each(selKeys, function(i, val){
			...
			$.each($.azOcrData.pageSearch ? $.azOcrData.pS[$.axZm.zoomID][val] : $.azOcrData.jsonObject[val], function(k, v){
				...
				*/
				...
				$('#axZm_zoomGal_'+v.P).css('display', 'block');
				if (firstMatch){
					$('#axZm_zoomGal_'+v.P).css('marginTop', 6);
					firstMatch = false;
				}
				$('#axZm_zoomFullGal_'+v.P).css('display', 'block');
