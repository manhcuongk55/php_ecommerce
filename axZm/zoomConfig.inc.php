...
// Ver. 4.2.5+ more than one license for several domains
...
$zoom['config']['version'] = '4.2.14';

// Ver. 4.2.5+ iso language, e.g. 'en'
...
// Ver. 4.2.1+ Possible values: false, filectime, filemtime
$zoom['config']['cTimeCompare'] = false;

// Ver. 4.1.9+ display dialog telling that cTimeCompare was used on a particular image
...
// Ver. 4.2.1+ array with additional inital images which are loaded depending on viewer size / resolution
$zoom['config']['stepPicDim'] = array(
	1 => array('w' => 900, 'h' => 900, 'q' => 75)
);

// Ver. 4.2.1+ if $zoom['config']['stepPicDim'] is not empty additional initial images will be predicted and preloaded
$zoom['config']['stepPicPreload'] = true;

// Ver. 4.2.1+ if $zoom['config']['stepPicDim'] is not empty use "more suited" preview images also on zoom
$zoom['config']['stepPicOnZoom'] = true;

// Ver. 4.2.1+, type of folder, subdolder structure for preview images, tiles and thumbnails
// possible values:
...
// Ver. 4.2.1+ Being at 100% zoom one more click will restore the zoom
...
// Ver. 4.2.1+ Background color for transparent areas of the png image
...
// Ver. 4.2.1+ regex for checking filename
$zoom['config']['regexFilename'] = "/^[a-zA-Z\_0-9]+([a-zA-Z\_0-9\-\.\,\(\)\[\]\%\s+]+)?\.+[a-zA-Z]{3,4}$/";

// Ver. 4.2.1+ regex for checking paths
$zoom['config']['regexPath'] = "/^[a-zA-Z\_0-9\:\/]+([a-zA-Z\_0-9\:\.\,\(\)\[\]\-\/\s+]*)$/";

// Ver. 4.2.1+ regex for checking file types (images magick could possibly process many other file types)
...
// 4.2.1+ "frontend" path which should be adjusted in some cases
...
// Map draggable or not, Ver. 4.2.1 changed default to false
...
// Ver. 4.2.1 changed default value to false
...
// Ver. 4.2.1+ not disabled for touch devices
...
// css .axZm_zoomGalleryBox to .axZm_zoomGalleryBoxOver AND .axZm_zoomFullGalleryBox to .axZm_zoomFullGalleryBoxOver
...
$zoom['config']['scrollPaneOptionsVert'] = array(
	'showArrows' => false, // Ver. 4.2.5 changed default value
	'mouseWheelSpeed' => 35,
	'animateScroll' => true,
...
// css: .axZm_zoomHorGalleryBox, .axZm_zoomHorGalleryBoxSelected, .axZm_zoomHorGalleryBoxOver
...
// Space between the thumb and outer frame (css: .axZm_zoomHorGalleryImg)
$zoom['config']['galHorCssPadding'] = 4; // integer (px)

// Border width outer frame (css: .axZm_zoomHorGalleryBox, .axZm_zoomHorGalleryBoxSelected, .axZm_zoomHorGalleryBoxOver)
$zoom['config']['galHorCssBorderWidth'] = 1; // integer (px)

// Height of description under the thumb (css: .axZm_zoomGalleryDescr)
$zoom['config']['galHorCssDescrHeight'] = 0; // integer (px)

// Padding of the description div (css: .axZm_zoomHorGalleryDescr)
...
// Thumbs Css: [.axZm_zoomGalleryBox, .axZm_zoomGalleryBoxSelected], .axZm_zoomGalleryBoxOver, .axZm_zoomGalleryImg, .axZm_zoomGalleryDescr
// Please do not use any width, margin or padding properties for this css in stylesheet file

// Space between the thumb and outer frame (css: .axZm_zoomGalleryImg)
$zoom['config']['galleryCssPadding'] = 8; // integer (px)

// Border width of the outer thumb frame in px (css: .axZm_zoomGalleryBox, .axZm_zoomGalleryBoxSelected, .axZm_zoomGalleryBoxOver)
$zoom['config']['galleryCssBorderWidth'] = 1; // integer (px)

// Height of description under the thumb (css: .axZm_zoomGalleryDescr)
$zoom['config']['galleryCssDescrHeight'] = 12; // integer (px)

// Padding within the description div (css: .axZm_zoomGalleryDescr)
$zoom['config']['galleryCssDescrPadding'] = 2; // integer (px)

// Distance between thumb "frames" (css: .axZm_zoomGalleryBox, .axZm_zoomGalleryBoxSelected, .axZm_zoomGalleryBoxOver)
...
// css .axZm_zoomFullGalleryBox, .axZm_zoomFullGalleryBoxOver, .axZm_zoomFullGalleryBoxSelected, .axZm_zoomFullGalleryDescr
...
// Thumbs Css: [.axZm_zoomFullGalleryBox, .axZm_zoomFullGalleryBoxSelected], .axZm_zoomFullGalleryBoxOver, .axZm_zoomFullGalleryImg, .axZm_zoomFullGalleryDescr
// Please do not use any width, margin or padding properties for this css in stylesheet file

// Space between the thumb and outer frame (css: zoomFullGalleryImg)
$zoom['config']['galFullCssPadding'] = 5; // integer (px)

// Border width outer frame (css: .axZm_zoomGalleryBox, .axZm_zoomFullGalleryBoxSelected, .axZm_zoomFullGalleryBoxOver)
$zoom['config']['galFullCssBorderWidth'] = 1; // integer (px)

// Height of description under the thumb (css: .axZm_zoomFullGalleryDescr)
$zoom['config']['galFullCssDescrHeight'] = 10; // integer (px)

// Padding within the description (css: .axZm_zoomFullGalleryDescr)
$zoom['config']['galFullCssDescrPadding'] = 2; // integer (px)

// Distance between thumb frames (css: .axZm_zoomFullGalleryBox, .axZm_zoomFullGalleryBoxSelected, .axZm_zoomFullGalleryBoxOver)
...
// Ver. 4.0+ Enable prev / next (prevnext) buttons above the picture of the player.
...
// Ver. 4.1.9+ Speed for gallerySlideNavi in ms if gallerySlideNaviAnm is set to SwipeHorz or SwipeVert
$zoom['config']['gallerySlideSwipeSpeed'] = 750;

// Ver. 4.2.14+ Settings for touch swipe which depends on real swipe speed gesture
...
// Ver. 4.1.9+ Reverse direction of the spin in general
...
// Ver. 4.2.1+ default value changed to false
...
// Ver. 4.2.5+ language vars spin preloader
...
// Ver. 4.2.4+ Circle spin preloader indicator, IE < 9 defaults to horizontal preloader above
// Ver. 4.2.5+ added text, L1 ... L5
$zoom['config']['spinCirclePreloader'] = array(
	...
	'stroke' => 5, // width of the progressive status line around the preloader, css: .axZm_circlePreloader_barCircle (color change there too)
...
$zoom['config']['spinSliderContainerHeight'] = 30; // int

// Padding of the slider container in px.
$zoom['config']['spinSliderContainerPadding'] = array('top'=>10, 'right'=>50, 'bottom'=>5, 'left' =>50);
...
// Ver. 4.2.5+ Css class name for spin slider
$zoom['config']['spinSliderClass'] = 'axZm_zoomSliderSpinContainer_light';

// Ver. 4.2.5+ Css class name for spin slider when it is over the player
$zoom['config']['spinSliderClassFS'] = 'axZm_zoomSliderSpinContainerFS';
...
// 4.2.1+ Max levels of 3D spin
$zoom['config']['spinMaxRows'] = 15;

// 4.2.1+ Max frames per row
$zoom['config']['spinMaxFrames'] = 360;

// 4.2.1+ Add image telling that the the user can drag and this way spin the 360
// class .axZm_dragToSpin
...
// 4.2.1+
// classes: .axZm_spinPreloadInit, .axZm_clickToSpin
...
$zoom['config']['spinNoInit'] = array(
	...
	'event' => 'click' // "click", "mouseenter" or false (trigger with $.fn.axZm.spinPreload() manually)
);

// Ver. 4.2.2+ if activated inertia force for 360 will be emulated when the user spins an object with the mouse or finger.
$zoom['config']['spinSmoothing'] = array(
	'enabled' => true, // enable / disable this option
	'glide' => 1, // the more, the longer it will spin
	'speed' => 1, // internal value
	'accMin' => 0.1, // internal value
	'accLimit' => 10 // internal value
);

// Ver. 4.2.3+ pan view vertically while spinning horizontally
$zoom['config']['spinAndDrag'] = true;

// Ver. 4.2.3+ if in spin mode right click and drag will pan instead of spin
$zoom['config']['spinPanRightMouseBtn'] = true;

// Ver. 4.2.4+ define which files should be not selected from a folder with 360 images.
...
// Ver. 4.2.3+ Disable "forceToPan" when the user chooses a different tool when "forceToPan" is enabled
...
// Ver. 4.2.5+ CSS class name for zoom slider
...
// Ver. 4.2.1+ changed default value
$zoom['config']['helpTxt'] = '
<DIV style="padding: 10px;">
	<h2 style="margin-top: 0">About: AJAX-ZOOM<h2>
</DIV>
'; // string
...
$zoom['config']['icons']['width'] = 32; // 4.2.3
$zoom['config']['icons']['height'] = 32; // 4.2.3
$zoom['config']['icons']['smallWidth'] = 21; // 4.2.3
$zoom['config']['icons']['smallHeight'] = 18; // 4.2.3
$zoom['config']['icons']['ext'] = 'png'; // 4.2.3
...
$zoom['config']['icons']['arrowLeft'] = array('file'=>'zoombutton_arrow_left', 'ext'=>'{ext}', 'w'=>31, 'h'=>94); // horizontal gallery
$zoom['config']['icons']['arrowRight'] = array('file'=>'zoombutton_arrow_right', 'ext'=>'{ext}', 'w'=>31, 'h'=>94); // horizontal gallery
...
$zoom['config']['icons']['mWidth'] = 50; // 4.2.3
$zoom['config']['icons']['mHeight'] = 50; // 4.2.3
$zoom['config']['icons']['mExt'] = 'png'; // 4.2.3
...
$zoom['config']['mNavi'] = array(
	...
	'onlyFullScreen' => false, // 4.2.1 display only at fullscreen mode (window)
	...
	'dis' => true, // Ver. 4.1.9+ disabled certain buttons like zoom in and out if they can not be used
	...
	'alt' => array (
		...
		'timeout' => 100,
		'fadeIn' => 100,
		'anm' => true, // 4.1.9+ animate
		'clas' => false, // 4.1.9+ change class of the tooltip, default axZm_zoomCustomNaviTooltip
...
// Ver. 4.2.1+ can be switched on runtime now ($.axZm.scroll = false;)
$zoom['config']['scroll'] = true;

// Enabling this option will not prevent scrolling the browser window with the mousewheel
// Ver. 4.2.1 can be switched on runtime now ($.axZm.mouseScrollEnable = true;)
...
// Progressbar height (css class .axZm_zoomBar)
...
// Ver. 4.2.1+ structure completly changed
$zoom['config']['cursor'] = array(
	...
	'spin360grabN' => array('openhand360N.cur', 'n-resize'), // spinFlip activated
...
// Ver. 4.2.1+ preload some images defined in css from /axZm/icons directory
...
$zoom['config']['innerMargin'] = 1; // interger (px)
...
// Ver. 4.1.9+ cache thumbs to disk
$zoom['config']['dynamicThumbsCache'] = true;

// Ver. 4.1.9+ allow change settings by GET parameter
$zoom['config']['dynamicThumbsCacheByGET'] = true;

// Ver. 4.1.9+ allow cropping images
$zoom['config']['dynamicThumbsAllowCrop'] = true;

// Ver. 4.1.9+ allow cropping images
$zoom['config']['dynamicThumbsCropMaxSize'] = 200;

// Ver. 4.1.9+ return croped images only if they are already cached
...
// Ver. 4.1.9+ quality range which can be passed over query string
$zoom['config']['dynamicThumbsQualRange'] = array(50, 95);

// Ver. 4.1.9+ compare creation time of image and thumb and if image is newer, recreate the thumb
$zoom['config']['dynamicThumbsCtime'] = false;

// Ver. 4.2.1+ default jpg quality
$zoom['config']['dynamicThumbsQual'] = 85;

// Ver. 4.2.1+ png quality
$zoom['config']['dynamicThumbsQualPng'] = array(
	'qual' => 9, // false (default) or int from 0 (no compression) to 9
	'filtering' => 5 // only Imagemagick: false (default) or int from 0 to 5. PNG data encoding filtering (before it is comressed) type: 0 is none, 1 is "sub", 2 is "up", 3 is "average", 4 is "Paeth", and 5 is "adaptive"
);

// Ver. 4.2.1, possible values - md5, char, flat (false)
...
// Ver. 4.2.1+ settings for querying images by the applications
...
// Ver. 4.2.1+ default path/image changed
...
// 4.2.2+ Watermark all over the image with $zoom['config']['wtrmrk']['file']
$zoom['config']['wtrmrk']['watermarkTilesFill'] = false;
...
// Ver. 4.2.1+ delete make.txt file which is generated while making the tiles and retry
...
// Ver. 4.1.9+ Adds divs for custom content around the player at fullscreen mode
...
// Ver. 4.2.5+ Scoll page on vertical touch motions when pan mode is activated and the image is not zoomed
// Ver. 4.2.12+ can be float - width of the player compared to window width
$zoom['config']['touchDragPageScroll'] = true;

// Ver. 4.2.12+ Scroll vertically if image is zoomed and pan mode activated
$zoom['config']['touchDragPageScrollZoomed'] = false;

// Ver. 4.2.5+ Scroll page on vertical touch motions when spin mode is activated
// Ver. 4.2.12+ can be float - width of the player compared to window width
$zoom['config']['touchSpinPageScroll'] = true;

// Ver. 4.2.12+ instantly deactivate vertical page scroll if this value is contained in the string of parent id of AJAX-ZOOM 
...
$zoom['config']['touchSettings'] = array(
	...
	'zoomSlider' => false,
	'spinSlider' => false,
	'zoomOutClick' => true // 4.2.1+
);

// Ver. 4.1.9+ Use window.requestAnimationFrame for jQuery animations;
...
// Ver. 4.2.1+ zoomify convert (only Unlimited)
...
// Ver. 4.2.12+ instantly set background color, useful if background color is solid e.g. white or black
...
// Ver. 4.2.2 conditions for include extended
...
if ($zoom['config']['visualConf']){
    if (file_exists(dirname(__FILE__).'/zoomVisualConf.inc.php')){include_once("zoomVisualConf.inc.php");}
}
...
$zoom['config']['fontPath']  = $axZmH->checkSlash($zoom['config']['urlPath'].'/'.$zoom['config']['fontPath'], 'add');
