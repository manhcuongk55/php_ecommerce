$(document).ready(function(){
	$(".list_item_product .picture").mouseover(function(){			
		$(this).find(".quick_intro").css({'display':'block','bottom': '0px'});			
	})

	$(".list_item_product").mouseout(function(){			
		$(this).find(".quick_intro").css({'display':'none', 'bottom': '0px'});
	})
	showdate();
	...
	// orders
	$(".div_send_order").click(function(){
		var name = $('.txt_name_cus').val().replace(/'/gi,'').replace(/"/gi,'');
		var sdt = $('.txt_sdt_cus').val().replace(/'/gi,'').replace(/"/gi,'');
		var add = $('.txt_add_cus').val().replace(/'/gi,'').replace(/"/gi,'');
		...
		$("#frm_sumit_order").submit();

	})

	// $("#static_menu li").mouseover(function(){
	// 	var left = $(".jcarousel ul").css('left');
	// 	$(".jcarousel").css({'position':'static'});
	// 	$(".jcarousel ul").css({'position':'static','margin-left':left});
	// })
	// $("#static_menu li").mouseout(function(){
	// 	$(".jcarousel").css('position','relative');
	// 	$(".jcarousel ul").css({'position':'relative','margin-left':'0px'});		
	// })

	$(".post_list_item").click(function(){
		var conId = $(this).attr('conId');
		$('html, body').animate({ scrollTop: $(".item_post_service."+conId).offset().top }, 2000);
	})

	$(".main_menu_product").click(function(){		
		$('html, body').animate({ scrollTop: $(".tab_list_catalog").offset().top }, 2000);
		// footer-widgets
	})	
	...
	$("#static_menu ul li").mouseover(function(){
		$(".flyout").css('display','none');
		$("#static_menu ul li").removeClass('active');
		cls = $(this).attr('class');
		catId = $(this).attr('id');
		$(this).addClass('active');
		$(".flyout."+cls+'.'+catId).css({'height':'368px', 'margin':'-5px auto', 'position':'absolute', 'text-indent':'0', 'width':'100%', 'display':'block'});
	})	
...
function showdate(){
	...
	var strtime = "<span class=date>"+dayarray[dayw]+", "+day+"/"+month+"/"+year+" "+hour+":"+minute+":"+second+"</span>"
	$("#gf-clock").html(strtime);
